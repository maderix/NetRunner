...
namespace DevionGames
{
    public class ModuleManagerWindow : EditorWindow
    {
        ...
        private string m_ModuleTxtPath = "https://deviongames.com/modules/modules.txt";
        ...
        private ModuleItem[] items= new ModuleItem[0];
        ...
        public static void ShowWindow()
        {
            ...
            window.StartBackgroundTask(window.RequestModules());
        }
        ...
        private void OnGUI()
        {
            ...
            {
                ModuleItem currentItem = items[i];
        ...
        protected static void OnScriptsReloaded()
        {
            ...
            if (windows.Length > 0) {
                windows[0].StartBackgroundTask(windows[0].RequestModules());
            }
        }
           ...
           // GUILayout.Space(9f);
            ...
            for (int i = 0; i < item.dependencies.Length; i++)
            {
                ...
                if (GUILayout.Button("<color=#007bff>" + item.dependencies[i]+"</color>",Styles.richTextLabel)) {
                    int index = Array.IndexOf(items,items.Where(x => x.id == item.dependencies[i]).FirstOrDefault());
                    GUI.FocusControl("");
                    selectedItem = items[index];
        ...
        private bool IsInstanlled(string moduleID) {
            ModuleItem item= items.Where(x => x.id == moduleID).FirstOrDefault();
            return item != null && item.IsInstalled;
        }

        private void StartBackgroundTask(IEnumerator update)
        {
        ...
        private void Import(ModuleItem item)
        {
            ...
            StartBackgroundTask(RequestModules());
        }
